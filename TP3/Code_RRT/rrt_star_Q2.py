...
def main():
    ...
    for i in range(3):
        ...
        for j in range(100):
            print(j)
            rrt_star = RrtStar(environment, x_start, x_goal, step_len, 0.10, 20, 3000)
            path, nb_iter = rrt_star.planning()
            
            # Add the number of iterations in the vector
            nb_iter_vec.append(nb_iter)

            if path:
                length_vec.append(get_path_length(path))
        ...
        print("Nb iter =", sum(nb_iter_vec)/len(nb_iter_vec))
        print("Path length =", sum(length_vec)/len(length_vec))
        print()
