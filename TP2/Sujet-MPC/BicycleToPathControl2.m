...
% La différence entre le tp précédent était que dans le précédent,
% le robot voyait le point le plus proche et tournait sa direction vers ce point.
% Maintenant, il voit plusieurs points, les met dans une liste et va au plus éloigné
% (c'est pour cela que le robot ne prend pas les angles, puisque lorsqu'il
% s'approche d'un waypoint, le robot va toujours au plus éloigné).
...
while size(list_points,2) < window_size
    ...
    error = dist(Path(:, goalWaypointId), xtemp);
    if norm(error(1:2)) < rho
        ...
        list_points = [list_points,xtemp]; %Next point in direction
    end
end
